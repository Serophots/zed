...
impl Editor {
    ...
    fn selection_replacement_ranges(
        ...
        let newest_selection = selections
            ...
            .unwrap();
        let start_delta = range.start - newest_selection.start;
        let end_delta = range.end - newest_selection.end;
        let snapshot = self.buffer.read(cx).read(cx);
        selections
            .into_iter()
            .map(|mut selection| {
                selection.start += start_delta;
                selection.end += end_delta;
                ...
                this.selections
                    ...
                    .find_map(|(selection, range)| {
                        if selection.id == newest_selection_id {
                            Some(
                                (range.start.0.0 as isize - selection.head().0 as isize)
                                    ..(range.end.0.0 as isize - selection.head().0 as isize),
                            )
                        } else {
                            None
                        }
